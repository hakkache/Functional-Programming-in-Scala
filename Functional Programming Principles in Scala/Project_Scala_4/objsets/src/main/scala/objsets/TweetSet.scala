...
abstract class TweetSet extends TweetSetInterface:
  ...
  def union(that: TweetSet): TweetSet = 
    var res = that
    this.foreach(tw => res = res.incl(tw))
    res
  ...
  def mostRetweeted: Tweet = 
    var mostRT: Tweet = null
    this.foreach(tw => 
      if mostRT == null || tw.retweets > mostRT.retweets then
        mostRT = tw
    )
    if mostRT == null then
      throw java.util.NoSuchElementException("mostRetweeted of empty set")
    else
      mostRT
  ...
  def descendingByRetweet: TweetList = 
    if this.isInstanceOf[Empty] then
      Nil
    else
